...
class SharedServiceStack(Stack):
    ...
    def create_lambda(self, region, account, repository_name, function_name):
        ...
        monorepo_lambda.add_to_role_policy(
            iam.PolicyStatement(resources=[f'arn:aws:ssm:{region}:{account}:parameter/SharedServiceTrigger/*'],
    ...
    def create_codecommit_repo(self, repository_name, branch_for_trigger):
        ...
        sample_bucket = s3.Bucket(self, 'SharedServiceBucket',
                                  removal_policy=RemovalPolicy.DESTROY,
                                  auto_delete_objects=True,
                                  bucket_name='shared-service-bucket-testmonoforviz')
        sample_deployment = s3_deployment.BucketDeployment(self, 'DeploySharedServiceBucket',
                                                           sources=[s3_deployment.Source.asset(tmp_dir)],
                                                           destination_bucket=sample_bucket)
        monorepo = codecommit.Repository(self, "SharedServiceRepo", repository_name=repository_name)
        cfn_repo = monorepo.node.find_child('Resource')
        cfn_repo.code = codecommit.CfnRepository.CodeProperty(s3={'bucket': sample_bucket.bucket_name, 'key': 'shared-service.zip'},
...
def zip_sample():
    ...
    tempdir = tempfile.mkdtemp('shared-service')
    with zipfile.ZipFile(os.path.join(tempdir, 'shared-service.zip'), 'w') as zf:
        # zf.writestr(f'monorepo-{branch_name}.json', json.dumps(codepipeline_map))
        for dirname, subdirs, files in os.walk("./shared-service/"):
            for filename in files:
                relativepath = os.path.join(dirname.replace("./shared-service/", ""), filename)
