...
curl -X POST -H 'Content-type:application/json' --data '{"text":"*'"$developerName"'* committed to *'"$gitRepo/$5"'* _('"$STAGE"')_","attachments":[{"color":"'"$buildColor"'","title":"'"$appName"'","title_link":"'"$repoPath"'","text":"*`'"$buildStatus"'`* <'"$codebuildPath"'|#'"$CODEBUILD_BUILD_NUMBER"'> ('"$gitBranch"')","fields":[{"title":"_'"$commitMessage"'_","value":"Author:<@'"$developerId"'|cal>","short":false}]}]}' $4;
